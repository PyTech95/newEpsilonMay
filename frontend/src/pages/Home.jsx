...
import {
  Sparkles, ArrowRight, LogIn, FileText, Download, ArrowUpRight,
  BookOpen, Calendar, Quote
} from 'lucide-react';
import { programs, cohorts, testimonials, LOGO_URL } from '../mock';
    ...
    <div>
      ...
      <section className="relative overflow-hidden bg-navy-deep text-cream min-h-[100vh]">
        ...
        <div className="relative container-x pt-40 md:pt-44 pb-24">
          ...
          <h1 className="font-display text-[2.6rem] sm:text-[3.6rem] md:text-[5.2rem] lg:text-[6rem] leading-[1.02] tracking-tight max-w-5xl fade-up">
            Turning technical fluency<br />
            <span className="italic font-editorial text-gold">into strategic value.</span>
          </h1>
      ...
      <section className="bg-cream py-24 md:py-32 relative overflow-hidden">
        ...
        <div className="container-x relative">
          ...
          <div className="mt-14 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-5 md:gap-6">
            {/* Brochure */}
            <div className="lg:col-span-2">
              <button className="relative group block w-full h-full text-left bg-navy text-cream p-7 md:p-9 overflow-hidden hover:bg-navy-deep transition-colors min-h-[200px]">
            ...
            <div className="lg:col-span-2">
              <a href="https://moodle.org/login/index.php" target="_blank" rel="noopener noreferrer"
                 className="relative group block w-full h-full bg-gold text-navy p-7 md:p-9 overflow-hidden hover:bg-[#d8b876] transition-colors min-h-[200px]">
            ...
            {/* Insights */}
            <Link to="/insights" className="lg:col-span-2 group block bg-cream border border-navy/15 hover:border-gold/60 transition-colors p-7 md:p-9 min-h-[200px]">
              <BookOpen size={22} className="text-gold mb-4" />
              <p className="eyebrow mb-3">Insights</p>
              <p className="font-display text-navy text-[1.2rem] md:text-[1.35rem] leading-tight group-hover:text-gold transition-colors">
                Frameworks &amp; field notes from faculty.
              </p>
              <span className="link-gold mt-5 inline-flex">Read Essays <ArrowUpRight size={13} /></span>
            </Link>

            {/* Events */}
            <Link to="/events" className="lg:col-span-2 group block bg-bone hover:bg-white transition-colors p-7 md:p-9 border border-navy/10 hover:border-gold/40 min-h-[200px]">
              <Calendar size={22} className="text-gold mb-4" />
              <p className="eyebrow mb-3">Free Events</p>
              <p className="font-display text-navy text-[1.2rem] md:text-[1.35rem] leading-tight group-hover:text-gold transition-colors">
                Live with us before you commit.
              </p>
              <span className="link-gold mt-5 inline-flex">View Schedule <ArrowUpRight size={13} /></span>
            </Link>

            {/* Featured programme card */}
            <div className="lg:col-span-4 relative overflow-hidden min-h-[340px] group">
              <img src="https://images.unsplash.com/photo-1551836022-d5d88e9218df?auto=format&fit=crop&w=1600&q=80"
                   alt="Featured programme" className="absolute inset-0 w-full h-full object-cover" />
              <div className="absolute inset-0 bg-gradient-to-r from-navy-deep/95 via-navy-deep/70 to-transparent" />
              <div className="relative p-8 md:p-10 text-cream flex flex-col justify-end h-full">
                <p className="eyebrow mb-3">Featured Programme</p>
                <h3 className="font-display text-[1.6rem] md:text-[2.2rem] leading-tight max-w-md">
                  Applied AI &amp; Machine Learning
                </h3>
                <p className="font-editorial italic text-cream/80 mt-3 text-lg">Turning Technical Fluency into Strategic Value</p>
                <div className="mt-5 flex items-center gap-6 text-sm">
                  <span className="font-display">₹1,25,000</span>
                  <span className="font-caps text-[0.65rem] text-gold">Cohort 04 · 15 March 2026</span>
                </div>
                <Link to="/programs/applied-ai-ml" className="link-gold mt-5 inline-flex w-fit">
                  Explore Programme <ArrowUpRight size={13} />
                </Link>
              </div>
            </div>

            {/* Upcoming cohorts */}
            <div className="lg:col-span-2 bg-bone border border-navy/10 p-7 md:p-9 min-h-[340px]">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                  <Calendar size={18} className="text-gold" />
                </div>
                <Link to="/admissions" className="link-gold">View <ArrowUpRight size={13} /></Link>
              </div>
              <p className="eyebrow mb-5">Upcoming Cohorts</p>
              <div className="space-y-5">
                {cohorts.map((c) => (
                  <div key={c.id} className="flex items-center justify-between border-b border-navy/10 pb-4 last:border-0 last:pb-0">
                    <div>
                      <p className="font-caps text-[0.65rem] text-gold">{c.label}</p>
                      <p className="font-display text-navy text-lg">{c.date}</p>
                    </div>
                    <span className="font-caps text-[0.6rem] text-navy/60">{c.status}</span>
                  </div>
                ))}
              </div>
            </div>
      ...
      <section className="bg-navy-deep text-cream py-24 md:py-32 relative overflow-hidden">
        ...
        <div className="container-x relative grid grid-cols-1 md:grid-cols-3 gap-10">
          {testimonials.map((t, i) => (
            <div key={i} className="border border-gold/15 p-8 bg-navy/40">
              <Quote size={20} className="text-gold mb-4" />
              <p className="font-editorial italic text-[1.25rem] leading-relaxed text-cream/90">“{t.quote}”</p>
        ...
        <div className="container-x">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-start mt-8">
            ...
            <p className="font-editorial text-navy/80 text-[1.2rem] leading-relaxed">
              Three evenings a week, you join your cohort live online — same screen, real conversation, faculty within reach. No pre-recorded sessions, no asynchronous drift.
            </p>
          </div>
          ...
          <div className="mt-14 grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="relative aspect-[4/3] overflow-hidden">
              ...
              <div className="absolute bottom-0 left-0 right-0 p-7 text-cream">
                ...
                <p className="font-display text-[1.4rem] leading-tight">3 sessions a week, 6 hours total</p>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-6">
              ...
              <div className="relative aspect-square overflow-hidden col-span-2">
                <img src="https://images.unsplash.com/photo-1675664535418-959dd68004fd?auto=format&fit=crop&w=1200&q=80" alt="" className="w-full h-full object-cover" />
                <div className="absolute inset-0 bg-gradient-to-t from-navy-deep/80 to-transparent" />
                <div className="absolute bottom-0 p-6 text-cream">
                  <p className="eyebrow mb-1">Capstone</p>
                  <p className="font-display text-[1.2rem] leading-tight">A portfolio that proves capability</p>
                </div>
              </div>
            </div>
          </div>
      ...
      <section className="relative bg-navy-deep text-cream py-28 md:py-36 overflow-hidden">
        ...
        <div className="relative container-x text-center">
          ...
          <h2 className="font-display text-[2.2rem] md:text-[3.6rem] leading-[1.05] max-w-3xl mx-auto mt-5">
